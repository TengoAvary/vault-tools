"""
embed_vault.py — index an Obsidian vault into a
SQLite vector store.
...
Creates <vault>/.vault-index/vault.db with a `chunks`
table:
...
Chunk modes:
    sliding (default) — overlapping character-window
    delimiter         — split on <!-- chunk --> markers
                        (for chatgpt_to_md.py output)
"""
...
def iter_text_files(vault: Path):
    """Yield .md and .txt file paths in the vault."""
    paths = sorted(
        [*vault.rglob("*.md"), *vault.rglob("*.txt")]
    )
    for path in paths:
...
def chunk_sliding(text: str) -> list[str]:
    ...
    # drop tiny tail fragments
    return [p for p in pieces if len(p) > 60]
...
def chunk_delimiter(text: str) -> list[str]:
    ...
    return [
        p.strip() for p in pieces if len(p.strip()) > 60
    ]
...
def _embed_files(
    ...
    """Chunk and embed a list of files.
    ...
    for path in files:
        ...
        for i, piece in enumerate(chunker(text)):
            all_rows.append(
                (str(path.relative_to(vault)), i, piece)
            )
...
def build_db(
    ...
    """Full rebuild of chunks and file_meta tables."""
    ...
    con.executemany(
        "INSERT INTO chunks"
        " (file_path, chunk_index, text, embedding)"
        " VALUES (?,?,?,?)",
        rows,
    )

    # Populate file_meta so incremental runs
    # have a baseline
    for path in files:
    ...
    con.close()
    print(
        f"Done. {len(rows)} chunks stored in {db_path}",
        file=sys.stderr,
    )
...
def incremental_update(
    ...
    for row in con.execute(
        "SELECT file_path, mtime, chunk_mode"
        " FROM file_meta"
    ):
    ...
    deleted = (
        set(stored.keys()) - set(current_files.keys())
    )
    ...
    for rel, path in current_files.items():
        ...
        if rel not in stored:
            ...
            mtime > stored[rel][0]
            or chunk_mode != stored[rel][1]
        ):
            changed.append((rel, path, mtime))

    if not deleted and not changed:
        print(
            "[embed] vault index is up to date",
            file=sys.stderr,
        )
        con.close()
        return
    ...
    if changed:
        print(
            f"[embed] re-indexing {len(changed)}"
            " file(s)...",
            file=sys.stderr,
        )
    ...
    print(
        f"[embed] done — {', '.join(parts)}",
        file=sys.stderr,
    )


def main():
    """Index an Obsidian vault into a SQLite vector store."""
    parser = argparse.ArgumentParser(
        description=(
            "Index an Obsidian vault into"
    ...
    parser.add_argument(
        "--chunk-mode",
        choices=["sliding", "delimiter"],
        default="sliding",
        help=(
            "Chunking strategy: 'sliding'"
            " (overlapping windows) or 'delimiter'"
    ...
    if args.full:
        ...
        incremental_update(
            vault, chunk_mode=args.chunk_mode
        )
